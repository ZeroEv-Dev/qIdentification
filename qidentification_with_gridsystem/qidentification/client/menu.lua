...
AddEventHandler('qidentification:requestLicense',function()
	...
	for i=1,#Config.IdentificationData,1 do 
		...
		table.insert(sendMenu,{
			id = #sendMenu + 1,
			header = "<span class='target-icon'><i class='fa-solid fa-id-card fa-fw'></i></span> Richiedi "..data.label,
			txt = "$"..data.cost,
			params = { 
	...
	{
		id = 99,
		header = "<span class='target-icon'><i class='fa-solid fa-circle-xmark fa-fw'></i></span> Cancella",
		txt = "",
		params = {
...
AddEventHandler('qidentification:applyForLicense',function(data)
	...
	--	local data = exports.ox_inventory:Keyboard('URL Custom (Leave blank for default)', {'Direct Image URL (link foto)'})
